...
rank = args.rank
print("rank is: ", rank)
print("init_method is: ", "tcp://" + args.addr + ":" + args.port)
...
def main():
    ...
    print('gen_chunk_sizes start')
    tx2_chunk_sizes = gen_chunk_sizes(world_size, datanum_idx)
    print('gen_chunk_sizes end')

    print('dist.send start')
    for i in range(world_size):
        dist.send(tx2_chunk_sizes[i], dst=i + 1)
    print('dist.send finish')
    ...
    FIG_ROOT_PATH = fig_dir + 'alpha_' + str(
        alpha) + 'model-type_' + args.model_type + '_dataset-type' + args.dataset_type + \
                    '_batch-size' + str(args.batch_size) + '_tx2nums' + str(args.world_size) + '_' + pattern_list[
                        args.pattern_idx] + '_data-pattern' + \
                    datanum_list[args.datanum_idx] + '_data' + '_exit-loss' + str(exit_loss_threshold) + '_lr' + str(
        args.lr) + '_epoch' + str(args.epochs) + '_local' + str(args.local_iters) + '_FIGTYPE.png'
    ...
    log_alpha_path = log_alpha_dir + 'alpha_' + str(alpha) + args.dataset_type + '_' + args.model_type + '_vm' + str(
        world_size) + '_' + pattern_list[args.pattern_idx] + '_data-pattern' + \
                     datanum_list[args.datanum_idx] + 'data' + \
                     '_lr' + str(args.lr) + '_epoch' + str(args.epochs) + \
                     '_vmtest' + str(args.enable_vm_test) + '_alpha.txt'
    log_agent_loss_path = log_alpha_dir + 'alpha_' + str(
        alpha) + args.dataset_type + '_' + args.model_type + '_vm' + str(world_size) + '_' + pattern_list[
                              args.pattern_idx] + '_data-pattern' + \
                          datanum_list[args.datanum_idx] + 'data' + \
                          '_lr' + str(args.lr) + '_epoch' + str(args.epochs) + \
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.alpha == 0.0 and epoch % update_interval == 0 and epoch > update_interval:
            ...
            if epoch > 2 * update_interval:
                ...
                actor_loss, critic_loss, entropy, total_loss = agent.train_model(
                    torch.Tensor([reward]).unsqueeze(0).to(device), torch.Tensor(states).unsqueeze(0).to(device))
